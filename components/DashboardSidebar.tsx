...
export default function DashboardSidebar({
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 flex flex-col border-gray-200 border-r bg-white transition-all duration-300 ease-in-out dark:border-gray-800 dark:bg-gray-900 ${
        isCollapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* Logo */}
      <div
        className={`flex items-center justify-center border-gray-200 border-b dark:border-gray-800 ${isCollapsed ? 'p-4' : 'p-6'}`}
      >
        <Link className="block" href={`/${locale}/dashboard`}>
          <span
      ...
      {/* Navigation */}
      <nav className={`flex-1 space-y-1 pt-3 ${isCollapsed ? 'px-2' : 'px-3'}`}>
