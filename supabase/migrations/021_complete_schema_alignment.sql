-- Migration: Complete Schema Alignment for September 2025
...
-- =====================================================
-- 7. CREATE OR REPLACE PROFILES VIEW
-- =====================================================

-- Drop existing view/table if it exists and recreate as view
DO $$
BEGIN
  -- Drop if exists as view
  DROP VIEW IF EXISTS public.profiles CASCADE;

  -- Check if profiles exists as a table
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema = 'public'
             AND table_name = 'profiles'
             AND table_type = 'BASE TABLE') THEN
    -- If it's a table, we need to handle it differently
    -- For now, skip creating the view
    RAISE NOTICE 'profiles exists as a table, skipping view creation';
  ELSE
    -- Create the view
...
-- =====================================================
-- 12. FUNCTIONS FOR BETTER PERFORMANCE
-- =====================================================
...
-- =====================================================
-- 14. SEED SAMPLE DATA FOR TESTING
-- =====================================================

-- Insert sample booking data if tables are empty
INSERT INTO public.bookings (user_id, provider_id, service_type, booking_date, booking_time, status, price)
SELECT
  u.id,
  sp.id,
  sp.services[1],
  CURRENT_DATE + (random() * 30)::INTEGER,
  ('09:00'::TIME + (random() * 10 || ' hours')::INTERVAL)::TIME,
  (ARRAY['pending', 'confirmed', 'completed'])[floor(random() * 3 + 1)]::booking_status,
  50 + random() * 200
FROM public.users u
...
-- Grant permission on profiles (whether it's a table or view)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema = 'public'
...
-- REFRESH MATERIALIZED VIEW CONCURRENTLY IF EXISTS ...;
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 021_complete_schema_alignment completed successfully!';
END $$;