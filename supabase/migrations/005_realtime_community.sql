...
-- Ensure community tables exist first
...
-- Add presence tracking table for online users
CREATE TABLE IF NOT EXISTS public.user_presence (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES public.users(id) ON DELETE CASCADE,
    channel TEXT NOT NULL, -- e.g., 'thread:uuid', 'community:main'
    status TEXT DEFAULT 'online', -- online, away, busy
    last_seen TIMESTAMPTZ DEFAULT NOW(),
    metadata JSONB DEFAULT '{}', -- Additional data like current_thread_id
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS public.typing_indicators (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES public.users(id) ON DELETE CASCADE,
    channel TEXT NOT NULL, -- 'thread:uuid' or 'dm:user1-user2'
...
-- Note: Cannot use NOW() in index WHERE clause as it's not immutable
...
-- Function to broadcast thread changes
...
AS $$
BEGIN
    -- Broadcast to community channel and specific thread channel
    ...
    -- Also broadcast to specific thread channel if it's a post
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        -- Update posts count and last post time
...
-- Create triggers for real-time broadcasts (DROP IF EXISTS first)
...
-- RLS Policies for Realtime (DROP IF EXISTS first)
