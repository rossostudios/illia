...
export function useSavedSearches(options: UseSavedSearchesOptions = {}) {
  ...
  const saveSearch = useCallback(
    async (name: string, filters: Record<string, any>, isDefault = false): Promise<boolean> => {
      if (!user) {
        return false
      }
  ...
  const getDefaultSearch = useCallback(
    (): SavedSearch | null => savedSearches.find((search) => search.is_default) || null,
    [savedSearches]
  )
