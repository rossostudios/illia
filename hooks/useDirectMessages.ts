...
export function useDirectMessages(options: UseDirectMessagesOptions = {}) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.includes('function') || error.code === '42883') {
          console.warn(
            'Direct messaging functions not yet created. Please run migration 007_enhanced_direct_messaging.sql'
          )
          setConversations([])
          return
        }
  ...
  const sendMessage = useCallback(
    async (text: string, type: 'text' | 'image' | 'file' = 'text', attachments?: any[]) => {
      if (!user || !options.otherUserId) return
  ...
  const sendTypingIndicator = useCallback(
    async (isTyping: boolean) => {
      if (!channelRef.current || !user || !options.otherUserId) return
