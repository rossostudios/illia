...
export function useAnalytics(dateRange?: DateRange) {
  ...
  const fetchEngagementMetrics = useCallback(
    async (startDate: Date, endDate: Date): Promise<UserEngagementMetrics> => {
      try {
        ...
        if (searchError) {
          console.warn('Search analytics table not found or error:', searchError)
        ...
        if (matchesError) {
          console.warn('Matches table not found or error:', matchesError)
        }
        ...
        if (leadsError) {
          console.warn('Leads table not found or error:', leadsError)
        }
  ...
  const fetchSearchAnalytics = useCallback(
    async (startDate: Date, endDate: Date): Promise<SearchAnalyticsData> => {
      try {
        ...
        if (error) {
          console.warn('Search analytics table not found or error:', error)
          return {
            popularSearchTerms: [],
  ...
  const fetchProviderPerformance = useCallback(async (): Promise<ProviderPerformanceData> => {
    try {
      ...
      if (error) {
        console.warn('Providers table not found or error:', error)
        return {
          totalProviders: 0,
          verifiedProviders: 0,
          avgRating: 0,
